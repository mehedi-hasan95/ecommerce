import { brandProductCountAction } from "@/actions/user/category-brand";
import { Badge } from "@/components/ui/badge";
import Image from "next/image";
import Link from "next/link";

const CategoriesPage = async () => {
  const data = await brandProductCountAction();
  return (
    <div className="max-w-screen-2xl mx-auto px-6 flex flex-wrap gap-8 pt-3 pb-10">
      {data.map((item) => (
        <Link
          href={`/brands/${item.id}`}
          key={item.id}
          className="flex flex-col items-center justify-center h-44 w-44 rounded-md group relative bg-themeOne"
        >
